...
const header = `<header class="topbar">${brand}<span class="version-label">${extension ? 'v' + chrome.runtime.getManifest().version : 'DESIGN PREVIEW'}</span>${isPopup ? `<button class="icon-button" id="open-settings" aria-label="Open all settings">${icon('sliders')}</button><a class="icon-button" href="https://github.com/camwooloo/streamshade" target="_blank" rel="noopener noreferrer" aria-label="Streamshade on GitHub" title="Streamshade on GitHub">${icon('link')}</a>` : '<span class="local-label"><i></i> MADE FOR YOUR MOMENT</span>'}</header>`;
